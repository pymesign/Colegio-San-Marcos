...
$input = '<input type="search" name="q" id="mod-finder-searchword' . $module->id . '" class="inputbox search-query input-medium js-finder-search-query form-control" value="' . htmlspecialchars($app->input->get('q', '', 'string'), ENT_COMPAT, 'UTF-8') . '"'
    . ' placeholder="' . Text::_('MOD_FINDER_SEARCH_VALUE') . '">';

$showLabel  = $params->get('show_label', 1);
$labelClass = (!$showLabel ? 'visually-hidden ' : '') . 'finder';
$label      = '<label for="mod-finder-searchword' . $module->id . '" class="element-invisible ' . $labelClass . '"></label>';

$output = '';

if ($params->get('show_button', 0)) {
    $output .= '<button class="button btn" type="submit"><i class="fas fa-search"></i></button>';
    $output .= $label;
...
?>
<div class="search">
<form class="form-inline mod-finder js-finder-searchform form-search" action="<?php echo Route::_($route); ?>" method="get" role="search">
    <?php echo $output; ?>
